...
const fetchRecord = async () => {
  fetching.value = true
  try {
    const res = await tripAssignService.getById(route.params.id)
...
const handleSubmit = async (formData) => {
  loading.value = true
  try {
    await tripAssignService.update(route.params.id, formData)
    message.success('Trip assignment updated successfully')
    router.push({ name: 'trips-list' })
  } catch (error) {
    message.error(error.response?.data?.message || 'Failed to update trip assignment')
  } finally {
    loading.value = false
  }
}
  ...
  <div class="max-w-4xl mx-auto space-y-6 pb-12">
    ...
    >
      <div class="flex items-center gap-4">
        ...
        >
          <LucideIcon name="CalendarHeart" :size="24" />
        </div>
        <div>
          <h1
          ...
          >
            Edit Trip Assignment
          </h1>
          <p
            class="text-sm mt-1 transition-colors duration-300"
            :class="themeStore.isDark ? 'text-gray-400' : 'text-gray-500'"
          >
            Modify selection for driver or dates for this trip
          </p>
        </div>
      </div>
    ...
    >
      ...
      <TripAssignForm ref="formRef" :record="record" :is-edit="true" @submit="handleSubmit" />
    </div>
  </div>
</template>
