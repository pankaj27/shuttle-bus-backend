...
const canCreate = computed(() => authStore.hasPermission('busschedule.create'))
const canEdit = computed(() => authStore.hasPermission('busschedule.edit'))
const canDelete = computed(() => authStore.hasPermission('busschedule.delete'))

const handleCreate = () => {
  if (authStore.isDemo) {
    Modal.warning({
      title: 'Action Restricted',
      content: 'Creating a new schedule is disabled in Demo Mode.',
    })
    return
  }
  if (canCreate.value) {
    router.push({ name: 'bus-schedule-create' })
  } else {
    message.error('You do not have permission to create schedules')
  }
}

const handleEdit = (record) => {
  if (canEdit.value) {
    router.push({ name: 'bus-schedule-edit', params: { id: record.id } })
  } else {
    message.error('You do not have permission to edit schedules')
  }
}
...
const onStatusChange = (val, record) => {
  if (authStore.isDemo) {
    ...
    Modal.warning({
      title: 'Action Restricted',
      content: 'Updating schedule status is disabled in Demo Mode.',
    })
    return
  }
  if (canEdit.value) {
    ...
    message.error('You do not have permission to update schedule status')
  }
}
...
const onDeleteConfirm = (record) => {
  ...
  if (canDelete.value) {
    ...
    message.error('You do not have permission to delete schedules')
  }
}
</script>
  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          >
            Manage bus timetables and routes schedule
          </p>
        </div>
      </div>
      <a-button
    ...
    >
      ...
      <a-tabs v-model:activeKey="activeTab" class="custom-tabs" @change="handleTabChange">
        <a-tab-pane key="all" tab="All Schedules" />
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <div class="flex justify-end gap-2">
              ...
              <a-tooltip :title="canCreate ? 'Copy' : 'No Permission to Copy'">
                <a-popconfirm
                  v-if="canCreate"
                  title="Are you sure you want to duplicate this schedule?"
                  ok-text="Yes"
                  cancel-text="No"
                  @confirm="handleCopy(record)"
                >
                  <a-button type="text" class="text-green-500 hover:bg-green-50 rounded-lg p-2">
                    <LucideIcon name="Copy" class="text-green-500" :size="16" />
                  </a-button>
                </a-popconfirm>
