...
onMounted(async () => {
  const data = await fetchSettings('privacypolicy');
  if (data) {
     form.value = data
  }
});
...
const handleSave = async () => {
  if (authStore.isDemo) {
    Modal.warning({
      ...
      content: 'Updating privacy policy is disabled in Demo Mode.',
    });
    return;
  }
  await updateSettings('privacypolicy', {privacypolicy: form.value});
};
const getPublicLink = () => {
    ...
    <a-form layout="vertical" class="mt-8">
      ...
      >
        <div class="flex justify-between items-center mb-6">
          ...
          >
            <LucideIcon name="Shield" :size="18" /> Policy Document
          </h4>
        ...
        <a-form-item label="Privacy Policy Text">
          <RichTextEditor
            v-model="form"
      ...
      <div class="flex flex-col items-end mt-12">
        <a-alert
          ...
          description="Privacy and data handling policies are locked in this demonstration."
          type="warning"
          show-icon
